...
for (var i = 0; i < dotsAmount; i++) {
  ...
  // vector.y = Math.sin(vector.phi)//**3;
  // vector.z = Math.sin(vector.theta) * Math.cos(vector.phi);

var n = 12
	vector.x = vector.theta*Math.sin(n*vector.theta)*(Math.cos(vector.phi)*.9)

	//0.2*(Math.sin(6*vector.theta))*(Math.cos(vector.phi)*Math.sin(vector.phi))**.8
	//*Math.sin(vector.theta)*(Math.exp(Math.cos(vector.theta))-2*Math.sin(vector.theta*4)-Math.sin(vector.theta/12)**5)
	vector.y =  vector.theta*Math.cos(n*vector.theta)*(Math.cos(vector.phi)*.9)
	//+Math.sin(vector.phi)
  vector.z = .15 //Math.sin(vector.theta) * Math.cos(vector.phi);
...
function animate(mouseEvent) {
  ...
  if (mouseEvent) {
    ...
    document.querySelector('canvas').style.filter = `hue-rotate(${document.hueangle}deg)`
